<?php
/*
Funktionen aus der PHP-Bibl.:

floatval(wert)              -> wandelt eine wert in eine Fließkommazahl um
ini_get(string)             -> liest eine php.ini-Einstellung
is_null(wert)               -> prüft auf den NULL-Wert 
number_format()             -> https://www.php.net/manual/de/function.number-format
...
function error_handling(&$e, $errno) {
    $e |= $errno;   // $error = $error | 0b1
    return null;
}
function getNumber(string $nr):float|int {
    if( str_ends_with($nr, 'h') ) {             // prüfung auf hex-eingabe                  0246afh
        $nr = substr($nr, 0, -1);               // lösche letztes zeichen
        $nr = hexdec($nr);                      // wandle string in dec um
    } elseif( str_ends_with($nr, 'b') ) {       // prüfung auf bin-eingabe                  1000101b
        $nr = substr($nr, 0, -1);               
        $nr = bindec($nr);
...
function getNumberType(string $nr):string {
    ...
    if( str_ends_with($nr, 'h') ) {             // prüfung auf hex-eingabe                  0246afh
        ...
        $type = '%.' . ( strlen($nr) - ( strpos($nr, ',') + 1 ) ) . 'f';        // 12,123 -> %.3f
    }
    return $type;
}
function saveData(string $z1, string $z2, string $co, float $result):void {
    // prüfe ob innerhalb der SESSION das Array für die Klakulationen noch nicht existiert
...
<body>
    ...
    <div>
        ...
        <div id="result" class="<?=$error?'error':'success'?>">
            ...
            if(isset($_POST['submit']) && !is_null($result) && !$error) {
                ...
                echo "<p>". decbin(intval($result)) ."<sub>bin</sub></p>";
                echo "<p>" . dechex(intval($result)) . "<sub>hex</sub></p>";
            } elseif($error) {
    ...
    -->                
    <?php
        // durchlaufe session letzter eintrag soll oben stehen
        if(array_key_exists('calculations', $_SESSION)) {
