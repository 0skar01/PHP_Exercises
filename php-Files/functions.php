...
<body>
    <h1>Funktionen</h1>
    <h2>Prozeduren (Funktion ohne Rückgabewert)</h2>
    <?php
    ...
    function power_3(int|float $basis, int $potenz = 2):void { // parameter $potenz ist optional; wird er nicht übergeben, wird 2 als Defaultwert verwendet
        echo pow($basis, $potenz) . "<br>";
    }
    ...
    <?php
    function power_4() {
        return pow(2, 3);               // return gibt einen Wert oder auch ein Array bzw. ein Objekt zurück
    }
    ...
    function variadisch(int|array ...$args) {
        foreach($args as $arg) {
            if(is_array($arg)){
                variadisch(...$arg);       // rekursiver aufruf der Funktion durch sich selbst
            }else{
                echo "$arg, ";
            }
        }
    }
    variadisch(1, 456, 2, 156 , 234, [34, 2342354, [1541,153161, [1456146,151456,14561456]]], 5465.54);
    ?>
